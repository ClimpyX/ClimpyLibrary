...
public class CrashProtectionListener implements Listener {
    ...
    public void onchat(AsyncPlayerChatEvent olay) {
        if (olay.getPlayer() instanceof Player) {
            ...
            if (olay.getMessage().contains("İ")) {
                ...
                oyuncu.sendMessage((ChatColor.RED + "Crash atma girişimi tespit edildi, sunucudaki yetkililere durum bildirildi."));
                log(oyuncu.getName() + " isimli oyuncu crash atmayı denedi. \n  &7[Açıklama] ► &cİşlem bilgileri: &e(CHAT UYARISI)");
            }
        }
    }
    ...
    public void oncommand(PlayerCommandPreprocessEvent olay) {
        if (olay.getPlayer() instanceof Player) {
            ...
            if (olay.getMessage().contains("İ")) {
                ...
                olay.setCancelled(true);
                oyuncu.sendMessage((ChatColor.RED + "Crash atma girişimi tespit edildi, sunucudaki yetkililere durum bildirildi."));
                log(oyuncu.getName() + " isimli oyuncu crash atmayı denedi. \n  &7[Açıklama] ► &cİşlem bilgileri: &e(KOMUT UYARISI)");
            }
        }
    }
        ...
        try {
            ...
            if (olay.getPlayer() instanceof Player &&
                ...
                if (itemadi != null &&
                    ...
                    oyuncu.sendMessage((ChatColor.RED + "Crash atma girişimi tespit edildi, sunucudaki yetkililere durum bildirildi."));
                    log(oyuncu.getName() + " isimli oyuncu crash atmayı denedi. \n  &7[Açıklama] ► &cİşlem bilgileri: &e(ITEM UYARISI)");
                }
                        ...
                        if (item != null) {
                            ...
                            if (meta != null &&
                                ...
                                if (displayName.contains("İ")) {
                                    ...
                                    e.setCancelled(true);
                                    player.sendMessage((ChatColor.RED + "Crash atma girişimi tespit edildi, sunucudaki yetkililere durum bildirildi."));
                                    log(player.getName() + " isimli oyuncu crash atmayı denedi. \n  &7[Açıklama] ► &cİşlem bilgileri: &e(ITEM UYARISI)");
                                }
                            }
                        }
                    }
    ...
    public void onsignchange(SignChangeEvent e) {
        for (int i = 0; i < 4; i++) {
            if (e.getLine(i).matches("^[a-zA-Z0-9_]*$")) {
                ...
                {
                    e.setCancelled(true);
                    e.getPlayer().sendMessage((ChatColor.RED + "Crash atma girişimi tespit edildi, sunucudaki yetkililere durum bildirildi."));
                    log(e.getPlayer().getName() + " isimli oyuncu crash atmayı denedi. \n  &7[Açıklama] ► &cİşlem bilgileri: &e(TABELA UYARISI)");
                }
    ...
    public void log(String message) {
        for(Player OPlar : getServer().getOnlinePlayers()) {
            if(OPlar.isOp()){
                OPlar.sendMessage(ChatColor.translateAlternateColorCodes('&', "§7[Log] ► §c " + message));
            }
        }
    }
}
